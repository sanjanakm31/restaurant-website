	...
	<section class="table_reservation_section">
        ...
        <div class="container">
            ...
            if(isset($_POST['submit_table_reservation_form']) && $_SERVER['REQUEST_METHOD'] === 'POST')
            {
                ...
                try
                {
                    $stmtgetCurrentClientID = $con->prepare("SELECT client_id FROM clients WHERE client_email = ?");
$stmtgetCurrentClientID->execute([$client_email]);
...
if (!$client_id) {
    ...
    $client_id = $con->lastInsertId(); // Get the new client ID
}
                    ...
                    echo "<div style='color: green; font-size: 20px; text-align: center; padding: 15px; border: 2px solid green; border-radius: 5px; background-color: #e8f5e9;'>
        🎉 Yay! Your reservation has been booked successfully! <br> 
